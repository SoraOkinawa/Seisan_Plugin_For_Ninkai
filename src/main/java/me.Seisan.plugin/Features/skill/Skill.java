...
public class Skill{
    ...
    public void launch(PlayerInfo playerInfo, Object... objects){
        ...
        message += "§c** " + p.getDisplayName() + " §r§créalise la technique "+name;
        int range = 50;
        takeMana(playerInfo);
        String newname = name;
        ...
        affichejutsu(p, playerInfo, range, messagecomponent, ItemUtil.createItemStack(Material.BOOK, 1, newname, getLore(p)), this.skillVisibility, this.needTarget, playerInfo.getTarget());
    ...
    public static void affichejutsu(Player p, PlayerInfo pInfo, int range, TextComponent messagecomponent, ItemStack item, boolean skillVisibility, boolean needTarget, Player target) {
        ...
        ComponentBuilder test = new ComponentBuilder()
                .append(TextComponent.fromLegacyText(messagecomponent.getText()))
                .event(new HoverEvent(HoverEvent.Action.SHOW_ITEM, texte));
        ...
        for (Entity entity : pInfo.getPlayer().getNearbyEntities(range, range, range)) {
            if (entity instanceof Player) {
                if (skillVisibility || entity.isOp() || (needTarget && entity.getName().equals(target.getName()))) {
                    entity.spigot().sendMessage(test.create());
                }
            }
        }
    }
